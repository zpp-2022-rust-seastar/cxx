...
fn expand(ffi: Module, doc: Doc, attrs: OtherAttrs, apis: &[Api], types: &Types) -> TokenStream {
    ...
    for (impl_key, &explicit_impl) in &types.impls {
        match *impl_key {
            ...
            ImplKey::SeastarSharedPtr(ident) => {
                expanded.extend(expand_seastar_shared_ptr(ident, types, explicit_impl));
            }
...
fn expand_seastar_shared_ptr(
    ...
    quote_spanned! {end_span=>
        #unsafe_token impl #impl_generics ::cxx::private::SeastarSharedPtrTarget for #ident #ty_generics {
            fn __typename(f: &mut ::cxx::core::fmt::Formatter<'_>) -> ::cxx::core::fmt::Result {
                f.write_str(#name)
            }
            unsafe fn __null(new: *mut ::cxx::core::ffi::c_void) {
